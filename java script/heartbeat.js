  ...
  rppg() {
    ...
    if (this.signal.length >= this.targetFps * this.windowSize) {
      ...
      if (!signal.empty()) {
        ...
        console.log(bpm);
        let elem = document.getElementById("bpm");
        elem.innerHTML = Math.round(bpm);
        let elm = document.getElementById("speed");
        if(bpm<50){
          elm.innerHTML = "Too Low";
        } else if(bpm>=60 && bpm<=100){
          elm.innerHTML = "Normal";
        } else if(bpm>100 && bpm <=140){
          elm.innerHTML = "Elevated";
        } else if(bpm>140 && bpm <=190){
          elm.innerHTML = "Too High";
        } else if(bpm>190){
          elm.innerHTML = "Critical";
        } else{
          elm.innerHTML = "No Pulse detected";
        }
        // Draw BPM
        this.drawBPM(bpm);
      }
